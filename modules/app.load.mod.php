...
*/
	...
	*/
		...
		} // _form_loaddetailretailload

		function _form_loaddetaildealerload($routerid=false,$formid=false) {
			global $applogin, $toolbars, $forms, $apptemplate, $appdb;
			...
			if(!empty($routerid)&&!empty($formid)) {
		      ...
		      $retval = array();
		      $retval['return_code'] = 'SUCCESS';
		      $retval['return_message'] = 'Customer dealer load successfully updated!';
		...
		function router() {
            ...
            if(!empty($this->post['datefrom'])&&!empty($this->post['dateto'])) {
              ...
              $where = " and extract(epoch from loadtransaction_createstamp)>=$datefrom and extract(epoch from loadtransaction_createstamp)<=$dateto";
						...
						if(!($result = $appdb->query("select *,(extract(epoch from now()) - extract(epoch from loadtransaction_updatestamp)) as elapsedtime from tbl_loadtransaction where loadtransaction_type='retail' and loadtransaction_retailload>0 $where order by loadtransaction_id desc"))) {
							json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
							die;
						}
						//pre(array('$result'=>$result));
          ...
          if($this->post['table']=='dealer') {
            ...
            if(!empty($this->post['datefrom'])&&!empty($this->post['dateto'])) {
              ...
              $where = " and extract(epoch from loadtransaction_createstamp)>=$datefrom and extract(epoch from loadtransaction_createstamp)<=$dateto";
						...
						if(!($result = $appdb->query("select *,(extract(epoch from now()) - extract(epoch from loadtransaction_updatestamp)) as elapsedtime from tbl_loadtransaction where loadtransaction_type='dealer' $where order by loadtransaction_id desc"))) {
							json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
							die;
						}
						//pre(array('$result'=>$result));
            ...
            if(!empty($this->post['datefrom'])&&!empty($this->post['dateto'])) {
              ...
              $where = " and extract(epoch from loadtransaction_createstamp)>=$datefrom and extract(epoch from loadtransaction_createstamp)<=$dateto";
						...
						if(!($result = $appdb->query("select *,(extract(epoch from now()) - extract(epoch from loadtransaction_updatestamp)) as elapsedtime from tbl_loadtransaction where loadtransaction_type='dealer' and loadtransaction_dealerload>0 $where order by loadtransaction_id desc"))) {
							json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
							die;
						}
						//pre(array('$result'=>$result));
