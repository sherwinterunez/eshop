...
if(!class_exists('APP_app_user')) {

	//class APP_app_user extends APP_App {
	class APP_app_user extends APP_Base_Ajax {
	
		var $desc = 'User Management';

		var $pathid = 'usermanagement';
		var $parent = false;
		...
		function add_rules() {
			...
			//$appaccess->rules($this->desc,'manage-users','Manage Users');
			//$appaccess->rules($this->desc,'manage-roles','Manage Roles');
		...
		function _form_patienthistory($routerid=false,$formid=false) {

			if(!empty($routerid)&&!empty($formid)) {

				$templatefile = $this->templatefile($routerid,$formid);

				$form = '<h1>This is form Patient History</h1>';

				if(file_exists($templatefile)) {
					return $this->_form_load_template($templatefile,$form);
				}
		...
		/*function _form_usermanagement($routerid=false,$formid=false) {
