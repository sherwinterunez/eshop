		      ...
		      die;
				...
				if(!empty($post['method'])&&($post['method']=='generatereport'||$post['method']=='generatereportprint')) {
					...
					if(!empty($params['smartmoneyinfo']['loadtransaction_id'])&&!empty($params['smartmoneyinfo']['loadtransaction_ymd'])) {
					  $receiptno = 'EN'.$params['smartmoneyinfo']['loadtransaction_ymd'] . sprintf('%0'.getOption('$RECEIPTDIGIT_SIZE',7).'d', intval($params['smartmoneyinfo']['loadtransaction_id']));
					}
...
//--- page #1 ----
					...
					$block[] = array(
						'type' => 'label',
						'label' => !empty($params['smartmoneyinfo']['loadtransaction_refnumber']) ? $params['smartmoneyinfo']['loadtransaction_refnumber'] : '',
						'labelWidth' => $labelWidthR,
						'className' => 'receiptDetails_'.$post['formval'],
					);
					...
					$block[] = array(
						'type' => 'label',
						'label' => !empty($params['smartmoneyinfo']['loadtransaction_receivername']) ? $params['smartmoneyinfo']['loadtransaction_receivername'] : '',
						'labelWidth' => $labelWidthR,
						'className' => 'receiptDetails_'.$post['formval'],
					);
					...
					$block[] = array(
						'type' => 'label',
						'label' => !empty($params['smartmoneyinfo']['loadtransaction_receiveraddress']) ? $params['smartmoneyinfo']['loadtransaction_receiveraddress'] : '',
						'labelWidth' => $labelWidthR,
						'className' => 'receiptDetails_'.$post['formval'],
					);
				...
				if(!empty($post['method'])&&$post['method']=='smartmoneysave'&&!empty($post['rowid'])) {
					...
					if(!empty($post['smartmoney_status'])) {
						...
						if(!empty($content['loadtransaction_status'])&&$content['loadtransaction_status']==TRN_COMPLETED_MANUALLY) {
							...
							$smartmoney_transactiontype = !empty($post['smartmoney_transactiontype']) ? $post['smartmoney_transactiontype'] : false;
							$smartmoney_oldtransactiontype = !empty($post['smartmoney_oldtransactiontype']) ? $post['smartmoney_oldtransactiontype'] : false;
							...
							if(!empty($loadtransaction_cardno)&&!empty($smartmoney_transactiontype)&&!empty($smartmoney_oldtransactiontype)&&$smartmoney_transactiontype!=$smartmoney_oldtransactiontype) {
								...
								$scontent = array();
								$scontent['smartmoneynumber_number'] = $loadtransaction_cardno;
								$scontent['smartmoneynumber_type'] = $smartmoney_transactiontype;

								if(!($result = $appdb->insert("tbl_smartmoneynumber",$scontent,"smartmoneynumber_id"))) {
									if(preg_match('/duplicate\s+key/si',$appdb->lasterror)) {
										if(!($result = $appdb->update("tbl_smartmoneynumber",$scontent,"smartmoneynumber_number='$loadtransaction_cardno'"))) {
											json_encode_return(array('error_code'=>123,'error_message'=>'Error in SQL execution.<br />'.$appdb->lasterror,'$appdb->lasterror'=>$appdb->lasterror,'$appdb->queries'=>$appdb->queries));
											die;
										}
									} else {
						...
						if(!empty($asm)) {
							...
							foreach($asm as $k=>$v) {
								if(!empty($v['smartmoney_modemcommand'])&&!empty($v['simcard_number'])&&$v['simcard_number']==$content['loadtransaction_assignedsim']) {
									...
									$content['loadtransaction_cardlabel'] = $v['smartmoney_label'];
									break;
								}
							}
						}
...
*/
				...
				if($post['method']=='smartmoneymanually') {
					...
					$params['tbDetails'][] = array(
						'type' => 'hidden',
						'name' => 'smartmoney_oldtransactiontype',
              ...
              $where = " and extract(epoch from loadtransaction_updatestamp)>=$datefrom and extract(epoch from loadtransaction_updatestamp)<=$dateto";
						...
						if(!empty($result['rows'][0]['loadtransaction_id'])) {
							...
							foreach($result['rows'] as $k=>$v) {
								...
								$cardno = getSmartMoneyCardNo($v['loadtransaction_assignedsim'],$v['loadtransaction_cardlabel']);
								...
								$rows[] = array('id'=>$v['loadtransaction_id'],'data'=>array(0,$v['loadtransaction_id'],pgDate($v['loadtransaction_updatestamp']),$receiptno,$v['loadtransaction_receivername'],$v['loadtransaction_refnumber'],$cardno,number_format($v['loadtransaction_amount'],2),number_format($v['loadtransaction_sendagentcommissionamount'],2),number_format($v['loadtransaction_transferfeeamount'],2),number_format($v['loadtransaction_receiveagentcommissionamount'],2),number_format($v['loadtransaction_otherchargesamount'],2),number_format($v['loadtransaction_amountdue'],2),$statusString));
							}
