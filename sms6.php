...
function modemInit() {
	...
	if($sms->sendMessageReadPort("AT+STGR=0,1,141\r\n", "\+STIN:\s+(\d+)\r\n")) {
		$result = $sms->getResult();
	...
	if(!empty($stin)&&$sms->sendMessageReadPort("AT+STGI=".$stin."\r\n", "(\+STGI:\s+.+)\r\nOK\r\n")) {
		$result = $sms->getResult();
		$result[1] = $sms->tocrlf($result[1]);
		print_r(array('history'=>$sms->history,'result'=>$result,'stin'=>$stin));
		$sms->clearHistory();
	...
	*/
	...
	if(!empty($menuid)&&$sms->sendMessageReadPort("AT+STGR=0,1,".$menuid."\r\n", "\+STIN:\s+(\d+)\r\n")) {
		$result = $sms->getResult();
		$result[1] = $stin = intval(trim($result[1]));
	...
	if(!empty($stin)&&$stin===6&&!empty($menuid)&&$sms->sendMessageReadPort("AT+STGR=".$stin.",1,".$menuid."\r\n", "\+STIN:\s+(\d+)\r\n")) {
		$result = $sms->getResult();
		$result[1] = $stin = intval(trim($result[1]));
	...
	if(!empty($stin)&&$stin===3&&$sms->sendMessageReadPort("AT+STGI=".$stin."\r\n", "(\+STGI:\s+.+)\r\nOK\r\n")) {
		$result = $sms->getResult();
		$result[1] = $sms->tocrlf($result[1]);
		print_r(array('history'=>$sms->history,'result'=>$result,'stin'=>$stin));
		$sms->clearHistory();
	...
	if(!empty($stin)&&$stin===3&&!empty($menuid)&&$sms->sendMessageReadPort("AT+STGR=".$stin.",1\r\n", "\>\r\n")) {
		$result = $sms->getResult();
		$result[0] = $sms->tocrlf($result[0]);
	...
	if(!empty($sendData)&&$stin===3&&$sms->sendMessageReadPort("123456".chr(26), "\r\nOK\r\n.+?\+STIN:\s+(\d+)\r\n")) {
		$result = $sms->getResult();
		$result[0] = $sms->tocrlf($result[0]);
		print_r(array('history'=>$sms->history,'result'=>$result));
		$sms->clearHistory();
	...
	/*if($sms->sendMessageReadPort("AT+STGI=0,1,141\r\n", "\+STIN:\s+(\d+)\r\n")) {
		$result = $sms->getResult();
		$result[1] = $sms->tocrlf($result[1]);
		print_r(array('history'=>$sms->history,'result'=>$result));
		$sms->clearHistory();
