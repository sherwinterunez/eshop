...
function getLoadTransaction($mode=0,$id=0) {
	global $appdb;
	...
	if(!($result = $appdb->query("select * from tbl_loadtransaction where $where loadtransaction_type in ('smartpadala','retail') order by loadtransaction_createstampunix desc"))) {
