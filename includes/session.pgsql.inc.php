...
if(!class_exists('APP_Session')) {
	...
	class APP_Session {
		...
		//var $lifetime = 120; // 3600secs = 60mins = 1hr
		...
		function start() {
			...
			if(empty($_COOKIE)||empty($_COOKIE[$this->sesname])) {
				...
				//if($ret = $this->query("select id,hash,extract(epoch from now()::timestamp without time zone) - extract(epoch from datestamp) as tt,extract(epoch from datestamp) as unixdate from tbl_session where hash='{$_COOKIE[$this->sesname]}'")) {
				if($ret = $this->query("select id,hash,extract(epoch from now()) - extract(epoch from datestamp) as tt,extract(epoch from datestamp) as unixdate from tbl_session where hash='{$_COOKIE[$this->sesname]}'")) {
					if($ret['affected']>0) {
						//$tt = time()-intval($ret['rows'][0]['unixdate']);
						$tt = intval($ret['rows'][0]['tt']);
						if($tt>$this->lifetime) {
							$this->_new();
						}
					} else {
