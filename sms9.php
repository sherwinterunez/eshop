...
function modemInit($dev=false) {
	...
	$simfunctions[] = array('command'=>'AT+CUSD=1,*343#','regx'=>array("\+CUSD\:.+?\r\n","(\d+)\:Smart\s+Prepaid"),'resultindex'=>1);

	$simfunctions[] = array('command'=>'AT+CUSD=1,%result%','regx'=>array("\+CUSD\:.+?\r\n","(Enter\s+number)"),'resultindex'=>0);

	$simfunctions[] = array('command'=>'AT+CUSD=1,09493621618','regx'=>array("\+CUSD\:.+?\r\n","(\d+)\:All\s+Text\s+Promos"),'resultindex'=>1);

	$simfunctions[] = array('command'=>'AT+CUSD=1,%result%','regx'=>array("\+CUSD\:.+?\r\n","(\d+)\:All\s+Text\s+30\s+2days"),'resultindex'=>1);
	...
	$simfunctions[] = array('command'=>'AT+CUSD=1,*343#','regx'=>array("\+CUSD\:\s+\d+\,\"(.+?)\"\,\d+\r\n","(\d+)\:Smart\s+Prepaid"),'resultindex'=>1);

	$simfunctions[] = array('command'=>'AT+CUSD=1,<result>','regx'=>array("\+CUSD\:\s+\d+\,\"(.+?)\"\,\d+\r\n","(Enter\s+number)"),'resultindex'=>0);

	$simfunctions[] = array('command'=>'AT+CUSD=1,09493621618','regx'=>array("\+CUSD\:\s+\d+\,\"(.+?)\"\,\d+\r\n","(\d+)\:Unli\s+Calls\s+\&\s+Texts"),'resultindex'=>1);

	$simfunctions[] = array('command'=>'AT+CUSD=1,<result>','regx'=>array("\+CUSD\:\s+\d+\,\"(.+?)\"\,\d+\r\n","(\d+)\:UCT\s+50\s+3days"),'resultindex'=>1);
	...
	*/
	...
	//$simfunctions[] = array('command'=>'AT+CMGL=4','regx'=>"(\+CMGL\:\s+\d+\,\d+.+?)\r\nOK\r\n",'resultindex'=>1);
...
modemInit("/dev/ttyUSB0");

//sendSMS("/dev/cu.usbserial",'214','?1515');
