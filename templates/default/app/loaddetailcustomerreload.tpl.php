...
<script>
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_%formval%() {
		...
		var formData2_%formval% = [
			...
			{type: "block", name: "tbPayments", hidden: true, width: 1150, blockOffset: 0, offsetTop:0, list:[]},
			//{type: "block", name: "tbMessage", hidden: true, width: 1150, blockOffset: 0, offsetTop:0, list:[]},
...
///////////////////////////////////
		...
		try {
			...
			var dhxCombo = myForm.getCombo("fund_recepientid");
	...
	<?php
			...
			$allCustomer = getAllRegularCustomer();
			...
			foreach($allCustomer as $k=>$v) {
				if($k!=$vars['post']['rowid']) {
					...
					if($v['customer_id']==$vars['params']['customerreloadinfo']['fund_recepientid']) {
						$selected = true;
					}
	...
	*/
			...
			dhxCombo.allcustomer = <?php echo json_encode($allCustomer); ?>;
			...
			dhxCombo.attachEvent("onClose", function(){
			    ...
			    try {
				    ...
				    var userid = myForm.getItemValue('fund_recepientid');

				    //console.log('userid: '+userid);
				    //console.log(JSON.stringify(this.allcustomer[userid].customer_mobileno));
				    //console.log(JSON.stringify(this.allcustomer[userid]));

				    myForm.setItemValue('fund_recepientnumber',this.allcustomer[userid].customer_mobileno);
				    myForm.setItemValue('fund_recepientpaymentterm',this.allcustomer[userid].customer_terms);
				    myForm.setItemValue('fund_recepientpaymentterm',this.allcustomer[userid].customer_terms);
		...
		myForm.setItemFocus("fund_recepientname");
...
///////////////////////////////////
		...
		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>save').onClick = function(id,formval,wid) {
			...
			var discount = myForm.getItemValue('fund_discount');
			var processingfee = myForm.getItemValue('fund_processingfee');
			var amount = myForm.getItemValue('fund_amount');

			myForm.setItemValue('fund_amountdue', amount);
