...
<script>
	...
	function messagingdetailsreplydetails_%formval%() {
		...
		var myForm = myForm2_%formval% = new dhtmlXForm("messagingdetailsreplydetailsform_%formval%",[]);
		...
		myComposeEditor.attachEvent("onAccess", function(eventName, evObj){
			var $ = jQuery;
			//showMessage("messagingdetailsreplydetails: "+eventName,5000);
		...
		myTab.toolbar.getToolbarData('messagingsendtooutbox').onClick = myTab.toolbar.getToolbarData('messagingsendnow').onClick = function(id,formval) {
			...
			var tonumbers = trim($("#messagingmainreplyform_%formval% input[name='txt_to_number']").val());

			var togroups = trim($("#messagingmainreplyform_%formval% input[name='txt_to_groups']").val());
			...
			var ports = trim($("#messagingmainreplyform_%formval% input[name='txt_ports']").val());
