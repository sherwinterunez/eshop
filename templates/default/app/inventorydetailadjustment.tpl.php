...
///////////////////////////////////////
		...
		var dhxCombo = myForm.getCombo("adjustment_customerreceipt");
		...
		dhxCombo.setTemplate({
			...
			columns: [
				...
				{header: "NO.", width:  150, css: "capital", option: "#receiptno#"},
				{header: "DATE", width:  150, css: "capital", option: "#receiptdate#"},
				{header: "CUSTOMER", width:  150, css: "capital", option: "#customer#"},
				{header: "ITEM", width:  150, css: "capital", option: "#item#"},
				{header: "TYPE", width:  150, css: "capital", option: "#type#"},
				{header: "STATUS", width:  150, css: "capital", option: "#status#"},
			]
		});
...
<?php
		...
		foreach($allLoadtransaction as $k=>$v) {
			if($k!=$vars['post']['rowid']) {
				...
				$receiptnum = $v['loadtransaction_ymd'] . sprintf('%0'.getOption('$RECEIPTDIGIT_SIZE',7).'d', intval($v['loadtransaction_id']));
				$receiptno = 'RL'.$receiptnum;

				$idx[] = $v['loadtransaction_id'];

				$v['receiptdate'] = pgDate($v['loadtransaction_createstamp']);
				$v['receiptno'] = $receiptno;
				$v['receiptnum'] = $receiptnum;

				$loadTran[$v['loadtransaction_id']] = $v;

				$opt[] = array('value'=>$v['loadtransaction_id'],'selected'=>$selected,'text'=>array(
					'id' => !empty($v['loadtransaction_id']) ? $v['loadtransaction_id'] : ' ',
					'receiptno' => $receiptno,
					'receiptdate' => pgDate($v['loadtransaction_createstamp']),
					'customer' => !empty($v['loadtransaction_customername']) ? $v['loadtransaction_customername'] : ' ',
					'item' => !empty($v['loadtransaction_item']) ? $v['loadtransaction_item'] : ' ',
					'type' => !empty($v['loadtransaction_type']) ? strtoupper($v['loadtransaction_type']) : ' ',
					'status' => !empty($v['loadtransaction_status']) ? getLoadTransactionStatusString($v['loadtransaction_status']) : ' '
				));
			}
		}
?>
*/
		...
		myForm.adjustment_customerreceipt = <?php echo json_encode($idx); ?>;

		myForm.adjustment_customerreceiptopt = <?php echo json_encode($loadTran); ?>;

		/*console.log(myForm.adjustment_customerreceiptopt);

		for(var prop in myForm.adjustment_customerreceiptopt) {
			console.log('ID: '+prop);
			console.log(myForm.adjustment_customerreceiptopt[prop]);
		}*/

		myForm.invalidReceiptNumber = false;
		...
		dhxCombo.attachEvent("onClose", function(){
			//console.log('onChange: '+value+', '+text);
			//console.log(this);
			//console.log('onClose: '+myForm.getItemValue('adjustment_customerreceipt'));
		});

		dhxCombo.attachEvent("onBlur", function(){
			//console.log('onChange: '+value+', '+text);
			var v = myForm.getItemValue('adjustment_customerreceipt');
			...
			if(myForm.adjustment_customerreceipt.indexOf(v)>=0) {
				//console.log('Valid '+myForm.adjustment_customerreceipt);
				myForm.invalidReceiptNumber = false;

				console.log(myForm.adjustment_customerreceiptopt[v]);

				var d = myForm.adjustment_customerreceiptopt[v];

				myForm.setItemValue('adjustment_customerreceiptno',d['receiptnum']);
				myForm.setItemValue('adjustment_customerid',d['loadtransaction_customerid']);
				myForm.setItemValue('adjustment_customername',d['loadtransaction_customername']);
				myForm.setItemValue('adjustment_customeritem',d['loadtransaction_item']);
				myForm.setItemValue('adjustment_customermobileno',d['loadtransaction_recipientnumber']);
				myForm.setItemValue('adjustment_customeramountdue',d['loadtransaction_amountdue']);
				myForm.setItemValue('adjustment_simcardassignment',d['loadtransaction_assignedsim']);
				myForm.setItemValue('adjustment_simcarddatetime',d['receiptdate']);
				myForm.setItemValue('adjustment_simcardreferenceno',d['loadtransaction_refnumber']);
				myForm.setItemValue('adjustment_simcarditembalance',d['loadtransaction_simcardbalance']);
				myForm.setItemValue('adjustment_simcardamountdue',d['loadtransaction_amountdue']);
				...
				myForm.checkItem('adjustment_forsimcard');
				myForm.enableItem('adjustment_blocksimcard');



			} else {
				myForm.invalidReceiptNumber = true;
				showAlert('Invalid Receipt Number!');
			}
		});

///////////////////////////////////////

		var dhxCombo = myForm.getCombo("adjustment_customerid");
		...
		dhxCombo.setTemplate({
			...
			columns: [
				...
				{header: "TYPE", width:  150, css: "capital", option: "#customertype#"},
			]
		});
...
*/
		...
		myForm.adjustment_customerid = <?php echo json_encode($idx); ?>;

		myForm.adjustment_customeropt = <?php echo json_encode($customer); ?>;

		myForm.invalidCustomerName = false;
		...
		dhxCombo.attachEvent("onClose", function(){
			//console.log('onChange: '+value+', '+text);
			//console.log(this);
			console.log('onClose: '+myForm.getItemValue('adjustment_customerid'));
		});

		dhxCombo.attachEvent("onBlur", function(){
			//console.log('onChange: '+value+', '+text);
			//console.log('onBlur: '+myForm.getItemValue('adjustment_customerid'));

			var v = myForm.getItemValue('adjustment_customerid');
			...
			//console.log(myForm.adjustment_customerreceipt);
			//console.log('indexOf '+myForm.adjustment_customerreceipt.indexOf(myForm.getItemValue('adjustment_customerreceipt')));
			if(myForm.adjustment_customerid.indexOf(v)>=0) {
				//console.log('Valid '+myForm.adjustment_customerreceipt);
				myForm.invalidCustomerName = false;

				var d = myForm.adjustment_customeropt[v];

				var r = myForm.getItemValue('adjustment_customerreceipt');

				if(r.length===0) {
					myForm.setItemValue('adjustment_customermobileno',d['customer_mobileno']);
				}

			} else {
				myForm.invalidCustomerName = true;
...
///////////////////////////////////////
		...
		dhxCombo.setTemplate({
			...
			columns: [
				/*{header: "&nbsp;",  width:  41, 		option: "#checkbox#"}, */ // column for checkboxes
				{header: "ID", width:  50, css: "capital", option: "#id#"},
				{header: "NAME", width:  150, css: "capital", option: "#name#"},
				{header: "NUMBER", width:  150, css: "capital", option: "#number#"},
				{header: "PROVIDER", width:  150, css: "capital", option: "#provider#"},
		...
		foreach($allSims as $k=>$v) {
			...
			if(!empty($v['simcard_number'])) {
				...
				$idx[] = $v['simcard_number'];

				$opt[] = array('value'=>$v['simcard_number'],'selected'=>$selected,'text'=>array(
					'id' => !empty($v['simcard_id']) ? $v['simcard_id'] : ' ',
					'name' => !empty($v['simcard_name']) ? $v['simcard_name'] : ' ',
					'number' => !empty($v['simcard_number']) ? $v['simcard_number'] : ' ',
					'provider' => !empty($v['simcard_provider']) ? $v['simcard_provider'] : ' ',
