...
///////////////////////////////////
		...
		var notificationopt = <?php echo json_encode($params['notificationopt']); ?>;

		var smscommands_checkprovidernotificationopt = <?php echo json_encode($params['smscommands_checkprovidernotification']); ?>;

		myForm.forEachItem(function(name){

		    var frmMatches = name.match(/smscommands\_key\_notifications\[(\d+)\]/);

		    if(frmMatches) {
		    	//showMessage(name+': '+frmMatches[1],5000);
					...
					dhxCombo.setTemplate({
						...
						columns: [
							...
							//{header: "ID", width:  150, css: "capital", option: "#notificationid#"},
							//{header: "NAME", width:  150, css: "capital", option: "#notificationname#"},
							//{header: "TYPE", width:  150, css: "capital", option: "#notificationtype#"},
							{header: "NOTIFICATION", width:  500, css: "capital", option: "#notificationvalue#"},
						]
					});

					dhxCombo.addOption(notificationopt[frmMatches[1]].opts);
				    dhxCombo.setComboText(notificationopt[frmMatches[1]].value);
				    dhxCombo.setComboValue(notificationopt[frmMatches[1]].value);

					dhxCombo.attachEvent("onClose", function(){
						var checked = dhxCombo.getChecked();
					    //your code here
					    //alert('combo closed: '+name);
					    dhxCombo.setComboText(checked);
					    dhxCombo.setComboValue(checked);
					    //myForm.setItemValue(name, 'hello, sherwin!');
		...
		var dhxCombo1 = myForm.getCombo('smscommands_checkprovidernotification');
		...
		dhxCombo1.addOption(smscommands_checkprovidernotificationopt.opts);
		dhxCombo1.setComboText(smscommands_checkprovidernotificationopt.value);
		dhxCombo1.setComboValue(smscommands_checkprovidernotificationopt.value);
...
///////////////////////////////////
		...
		myForm.attachEvent("onChange", function (name, value){
		    ...
		    var chkMatches = name.match(/smscommands\_key\[(\d+)\]/);
		    ...
		    if(chkMatches) {
			    if(value) {
				    ...
				    if(val.o) {
				    	myForm.setItemValue('smscommands_key_['+chkMatches[1]+']',val.o);
				    } else {
				    	myForm.setItemValue('smscommands_key_['+chkMatches[1]+']','');
				    }
			    } else {
			    	myForm.setItemValue('smscommands_key_['+chkMatches[1]+']','');
			    }
		    ...
		    var chkMatches = name.match(/smscommands\_key\_error\[(\d+)\]/);
		    ...
		    if(chkMatches) {
			    if(value) {
				    ...
				    if(val.o) {
				    	myForm.setItemValue('smscommands_key_error_['+chkMatches[1]+']',val.o);
				    } else {
				    	myForm.setItemValue('smscommands_key_error_['+chkMatches[1]+']','');
				    }
			    } else {
			    	myForm.setItemValue('smscommands_key_error_['+chkMatches[1]+']','');
			    }
