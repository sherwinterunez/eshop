...
$myToolbar = array($moduleid.'edit',$moduleid.'clone',$moduleid.'delete',$moduleid.'save',$moduleid.'cancel',$moduleid.'refresh',$moduleid.'insertrow');
...
///////////////////////////////////
		...
		for(var i=0;i<20;i++) {
			jQuery("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% input[name='atcommands_resultindex["+i+"]']").numeric();
			jQuery("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% input[name='atcommands_repeat["+i+"]']").numeric();
		}
		...
		myWinToolbar.disableOnly(['<?php echo $moduleid; ?>save','<?php echo $moduleid; ?>cancel','<?php echo $moduleid; ?>insertrow']);
		...
		myWinToolbar.disableItem('<?php echo $moduleid; ?>clone');
...
///////////////////////////////////
		...
		myForm.attachEvent("onChange", function (name, value){
		    ...
		    var chkMatches = name.match(/atcommands\_regx(\d+)\[(\d+)\]/);
		    ...
		    if(chkMatches&&value) {
			    ...
			    if(val.o) {
			    	myForm.setItemValue('options_regx'+chkMatches[1]+'['+chkMatches[2]+']',val.o);
			    } else {
			    	myForm.setItemValue('options_regx'+chkMatches[1]+'['+chkMatches[2]+']','');
			    }
		...
		myForm.attachEvent("onFocus", function(name, value){
			...
			//showMessage("onFocus: name => "+name+", value => "+value,5000);

			var chkMatches = name.match(/atcommands\_at\[(\d+)\]/);

			if(chkMatches) {
				if(myFormStatus_%formval%=='settingnew'||myFormStatus_%formval%=='settingedit') {
					myForm.MyInsertRow = parseInt(chkMatches[1])+1;
					myWinToolbar.enableItem('settinginsertrow');
					//console.log({chkMatches:chkMatches,status:myFormStatus_%formval%,MyInsertRow:myForm.MyInsertRow});
				}
		...
		myForm.attachEvent("onBlur", function(name){
			...
			//}
			if(myForm.MyInsertRow) {
				//console.log({MyInsertRow:myForm.MyInsertRow});
				myWinToolbar.disableItem('settinginsertrow');
			}
		});

		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>insertrow').onClick = function(id,formval,wid) {
			//showMessage("toolbar: "+id,5000);

			if(!srt.windows[wid].form.MyInsertRow) {
				return false;
			}

			//console.log({MyInsertRow:srt.windows[wid].form.MyInsertRow});

			var rowid = srt.windows[wid].form.getItemValue('rowid');

			myTab.postData('/'+settings.router_id+'/json/', {
				odata: {rowid:rowid, wid:wid},
				pdata: "routerid="+settings.router_id+"&action=formonly&formid=<?php echo $templatedetailid.$submod; ?>&module=<?php echo $moduleid; ?>&method="+myFormStatus_%formval%+"&rowid="+rowid+"&formval="+formval+"&wid="+wid+"&insertrow="+srt.windows[wid].form.MyInsertRow,
			}, function(ddata,odata){
				if(ddata.html) {
