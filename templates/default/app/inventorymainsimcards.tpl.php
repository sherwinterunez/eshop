...
$myToolbar = array($moduleid.'new',$moduleid.'refresh',$moduleid.'pause',$moduleid.'resume',$moduleid.'restart');
...
///////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			...
			myGrid.setHeader("#master_checkbox, Product ID, Name, Number, Provider, Category, IP Address, Signal, Port, Balance, Active, Online, Paused");

			myGrid.setInitWidths("50,100,*,120,120,120,120,120,120,120,80,80,80");

			myGrid.setColAlign("center,center,left,left,left,left,center,center,center,right,center,center,center");
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachHeader("&nbsp;,#text_filter,#text_filter,#text_filter,#text_filter,#combo_filter,#text_filter,#text_filter,#text_filter,#text_filter,#combo_filter,#combo_filter,#combo_filter");
		...
		myTab.toolbar.getToolbarData('<?php echo $moduleid; ?>pause').onClick = myTab.toolbar.getToolbarData('<?php echo $moduleid; ?>resume').onClick = myTab.toolbar.getToolbarData('<?php echo $moduleid; ?>restart').onClick = function(id,formval) {
			//showMessage("toolbar: "+id,5000);
			//doSelect_%formval%("retail");

			myTab.postData('/'+settings.router_id+'/json/', {
				odata: {},
				pdata: "routerid="+settings.router_id+"&action=formonly&formid=<?php echo $templatedetailid.$submod; ?>&module=<?php echo $moduleid; ?>&method="+id+"&formval="+formval+"&wid=none",
			}, function(ddata,odata){
				//if(ddata.html) {
					//jQuery("#formdiv_%formval% #<?php echo $templatedetailid; ?>").parent().html(ddata.html);
					//if(ddata.html) {
		...
		/*myTab.toolbar.getToolbarData('<?php echo $moduleid; ?>resume').onClick = function(id,formval) {
			//showMessage("toolbar: "+id,5000);
			//doSelect_%formval%("retail");

		};

		myTab.toolbar.getToolbarData('<?php echo $moduleid; ?>restart').onClick = function(id,formval) {
			//showMessage("toolbar: "+id,5000);
			//doSelect_%formval%("retail");
