...
///////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			...
			myGrid.setHeader("#master_checkbox,ID, Purchase Order Date, Purchase Order Number, Supplier, Status");

			myGrid.setInitWidths("50,70,200,200,200,200");
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachEvent("onRowDblClicked", function(rowId,cellIndex){
						...
						obj.title = 'Purchase Order / '+myGrid.cells(rowId,2).getValue()+' / '+myGrid.cells(rowId,3).getValue()+' / '+myGrid.cells(rowId,4).getValue();
