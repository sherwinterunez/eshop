...
////////
		...
		if($("#formdiv_%formval% #<?php echo $templatemainid; ?>reportsettinggrid").length) {

			$("#formdiv_%formval% #<?php echo $templatemainid; ?>reportsettinggrid").height(lbHeight-90);
			$("#formdiv_%formval% #<?php echo $templatemainid; ?>reportsettinggrid").width(lbWidth);
		...
		if($("#formdiv_%formval% #<?php echo $templatedetailid; ?>reportsetting").length) {

			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reportsetting").height(lcHeight-2);
			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reportsetting").width(lcWidth-2);

			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reportsettingdetailsform_%formval%").height(lcHeight-51);
			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reportsettingdetailsform_%formval%").width(lcWidth-22);
		}

////////

		if($("#formdiv_%formval% #<?php echo $templatemainid; ?>reporttocsvgrid").length) {

			$("#formdiv_%formval% #<?php echo $templatemainid; ?>reporttocsvgrid").height(lbHeight-90);
			$("#formdiv_%formval% #<?php echo $templatemainid; ?>reporttocsvgrid").width(lbWidth);
		...
		if($("#formdiv_%formval% #<?php echo $templatedetailid; ?>reporttocsv").length) {

			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reporttocsv").height(lcHeight-2);
			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reporttocsv").width(lcWidth-2);

			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reporttocsvdetailsform_%formval%").height(lcHeight-51);
			$("#formdiv_%formval% #<?php echo $templatedetailid; ?>reporttocsvdetailsform_%formval%").width(lcWidth-22);
		}
...
////////
	...
	function explorer_sidebar_%formval%(){
		...
		mySideBar = mySideBar_%formval% = myTab.layout.cells('a').attachSidebar({
			...
			items: [
					{id: "reportsetting", text: "Report Setting", icon: "recent.png"},
					{id: "reporttocsv", text: "Export to CSV", icon: "desktop.png"},
		...
		setTimeout(function(){
			doSelect_%formval%("reportsetting");
		},100);
	...
	function doSelect_%formval%(id, lastId){
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=formonly&formid=reportsmain"+id+"&module=reports&formval=%formval%",
		}, function(ddata,odata){
			if(ddata.html) {
				jQuery("#formdiv_%formval% #reportsmain").parent().html(ddata.html);				
			}
		});
