...
<div id="messagingdetails">
	<div id="messagingdetailsports" class="navbar-default-bg"><div id="messagingdetailsportsdetailsform_%formval%"></div></div>
				...
				showConfirmWarning('Are you sure you want to delete this Port?',function(val){
					...
					if(val) {
						myTab.postData('/'+settings.router_id+'/json/', {
							...
							if(ddata.return_code&&ddata.return_message) {
				...
				success: function(data, statusText, xhr, $form, obj){
					...
					if(data.return_code) {
						if(data.return_code=='SUCCESS') {
							...
							if(data.rowid) {
								layout_resize_%formval%(data.rowid);
							} else {
