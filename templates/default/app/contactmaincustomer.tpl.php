...
///////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			...
			myGrid.setHeader("#master_checkbox,Customer ID, Primary Mobile No, Last Name, First Name, Middle Name, Type, Suffix");
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachEvent("onRowDblClicked", function(rowId,cellIndex){
						...
						obj.title = 'Customer / '+myGrid.cells(rowId,2).getValue()+' / '+myGrid.cells(rowId,3).getValue()+' / '+myGrid.cells(rowId,4).getValue()+' / '+myGrid.cells(rowId,5).getValue();
