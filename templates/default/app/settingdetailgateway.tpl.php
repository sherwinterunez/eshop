...
//$myToolbar = array($moduleid.'new',$moduleid.'edit',$moduleid.'clone',$moduleid.'delete',$moduleid.'save',$moduleid.'cancel',$moduleid.'refresh');

$myToolbar = array($moduleid.'edit',$moduleid.'clone',$moduleid.'delete',$moduleid.'save',$moduleid.'cancel',$moduleid.'refresh');
...
///////////////////////////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&table=gatewaylist&rowid=<?php echo !empty($vars['post']['rowid'])?$vars['post']['rowid']:'0'; ?>&formval=%formval%",
		}, function(ddata,odata) {

			if(typeof(myWinObj.myGridGatewayList)!='null'&&typeof(myWinObj.myGridGatewayList)!='undefined'&&myWinObj.myGridGatewayList!=null) {
				try {
					myWinObj.myGridGatewayList.destructor();
					myWinObj.myGridGatewayList = null;
				} catch(e) {
					console.log(e);
				}
			}

			var myGridGatewayList = myWinObj.myGridGatewayList = new dhtmlXGridObject(myForm.getContainer('gatewaylist_container'));

			myGridGatewayList.setImagePath("/codebase/imgs/")

			myGridGatewayList.setHeader("SEQ, SIM CARD NAME, SIM CARD NUMBER, FAILED, &nbsp;");

			myGridGatewayList.setInitWidths("50,150,150,100,*");

			myGridGatewayList.setColAlign("center,left,left,center,left");

			myGridGatewayList.setColTypes("edn,combo,ro,ch,ro");

			myGridGatewayList.setColSorting("str,str,str,str,str");

			myGridGatewayList.init();

			myGridGatewayList.setSizes();

			/*
			var myCombo = myGridGatewayList.getColumnCombo(3);
			...
			*/
			...
			/*myGridGatewayList.addRow(1,"1,SMART1618");
			myGridGatewayList.addRow(2,"2,");
			myGridGatewayList.addRow(3,"3,");
			myGridGatewayList.addRow(4,"4,");
			myGridGatewayList.addRow(5,"5,");
			myGridGatewayList.addRow(6,"6,");
			myGridGatewayList.addRow(7,"7,");
			myGridGatewayList.addRow(8,"8,");
			myGridGatewayList.addRow(9,"9,");
			myGridGatewayList.addRow(10,"10,");*/

			myGridGatewayList.attachEvent("onEditCell", function(stage,rId,cInd,nValue,oValue){
				//showMessage('state=>'+stage+', rId=>'+rId+', cInd=>'+cInd+', nValue=>'+nValue+', oValue=>'+oValue,10000);

				if(stage==1&&cInd==0) {
					myGridGatewayList.cells(rId,cInd).inputMask({mask:'99999999999',placeholder:''});
				}
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGridGatewayList.parse(ddata,function(){

						/*if(typeof(f)!='undefined'&&rowid!=null) {
							myGridGatewayList.selectRowById(rowid,false,true,true);
						} else
						if(typeof(f)=='undefined'&&ddata.rows.length>0) {
							myGridGatewayList.selectRowById(ddata.rows[0].id,false,true,true);
						}*/

						<?php if(!($method==$moduleid.'new'||$method==$moduleid.'edit')) { ?> 

						myGridGatewayList.forEachRow(function(id){
							myGridGatewayList.cells(id,0).setDisabled(true);
							myGridGatewayList.cells(id,1).setDisabled(true);
							myGridGatewayList.cells(id,2).setDisabled(true);
							myGridGatewayList.cells(id,3).setDisabled(true);
						});
						...
						if(ddata.rows&&ddata.rows.length>0) {
							for(x in ddata.rows) {
								if(ddata.rows[x].options) {
									...
									var myCombo = myGridGatewayList.getColumnCombo(1);
...
///////////////////////////////////
		...
		myForm.setItemFocus("gateway_desc");
...
///////////////////////////////////
		...
		/*myWinToolbar.getToolbarData('<?php echo $moduleid; ?>clone').onClick = function(id,formval,wid) {
			//showMessage("toolbar: "+id,5000);
		...
		myWinToolbar.getToolbarData('<?php echo $moduleid; ?>save').onClick = function(id,formval,wid) {
			...
			myWinObj.myGridGatewayList.forEachRow(function(id){
				var m = myWinObj.myGridGatewayList.cells(id,0).getValue();
				var n = myWinObj.myGridGatewayList.cells(id,1).getValue();
				var o = myWinObj.myGridGatewayList.cells(id,3).getValue();
				if(m&&n) {
					extra['gatewaylist_seq['+id+']'] = m;
					extra['gatewaylist_simname['+id+']'] = n;
					extra['gatewaylist_failed['+id+']'] = o;
					//extra['itemassignedsim_simname['+id+']'] = myGridGatewayList_%formval%.cells(id,2).getValue();
					//extra['itemassignedsim_simcommand['+id+']'] = myGridGatewayList_%formval%.cells(id,3).getValue();
				}
			});
