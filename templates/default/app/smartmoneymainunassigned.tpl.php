...
///////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&table=unassigned&formval=%formval%&datefrom="+encodeURIComponent(datefrom)+"&dateto="+encodeURIComponent(dateto),
			...
			try {
				...
				if(ddata.rows[0].id) {
					...
					myGrid.attachEvent("onRowDblClicked", function(rowId,cellIndex){
						...
						obj.title = 'Unassigned / '+myGrid.cells(rowId,3).getValue()+' / '+myGrid.cells(rowId,5).getValue();
