...
<script>
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_resize_%formval%(myWinObj) {
		...
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .stock_simcards_%formval% .dhxform_container").height(dim[1]-150);
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .stock_simcards_%formval% .dhxform_container").width(dim[0]-54);

		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .stock_products_%formval% .dhxform_container").height(dim[1]-150);
		$("#<?php echo $wid.$templatedetailid.$submod; ?>detailsform_%formval% .stock_products_%formval% .dhxform_container").width(dim[0]-54);


		if(typeof(myWinObj.mySimcardsGrid)!='undefined') {
			try {
				myWinObj.mySimcardsGrid.setSizes();
			} catch(e) {}
		}

		if(typeof(myWinObj.myProductsGrid)!='undefined') {
			try {
				myWinObj.myProductsGrid.setSizes();
			} catch(e) {}
		}
	...
	function <?php echo $wid.$templatedetailid.$submod; ?>_%formval%() {
		...
		myTabbar.addTab("tbDistribution", "Account Distribution");
		...
		var formData2_%formval% = [
			...
			{type: "block", name: "tbSimcards", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbSimcards']) ? json_encode($params['tbSimcards']) : '[]'; ?>},
			{type: "block", name: "tbProducts", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbProducts']) ? json_encode($params['tbProducts']) : '[]'; ?>},
			{type: "block", name: "tbHistory", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbHistory']) ? json_encode($params['tbHistory']) : '[]'; ?>},
			{type: "block", name: "tbDistribution", hidden:false, width: 1150, blockOffset: 0, offsetTop:0, list:<?php echo !empty($params['tbDistribution']) ? json_encode($params['tbDistribution']) : '[]'; ?>},
			{type: "label", label: ""}
		];
		...
		myForm.hideItem('tbProducts');
		myForm.hideItem('tbHistory');
		myForm.hideItem('tbDistribution');

///////////////////////////////////

		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&table=simcard&rowid=<?php echo !empty($vars['post']['rowid'])?$vars['post']['rowid']:'0'; ?>&formval=%formval%",
		}, function(ddata,odata){

			if(typeof(myWinObj.mySimcardsGrid)!='null'&&typeof(myWinObj.mySimcardsGrid)!='undefined'&&myWinObj.mySimcardsGrid!=null) {
				try {
					myWinObj.mySimcardsGrid.destructor();
					myWinObj.mySimcardsGrid = null;
				} catch(e) {
					console.log(e);
				}
			}

			var mySimcardsGrid = myWinObj.mySimcardsGrid = new dhtmlXGridObject(myForm.getContainer('stock_simcards'));

			mySimcardsGrid.setImagePath("/codebase/imgs/")

			mySimcardsGrid.setHeader("SIM ID No., SIM Card Name, Quantity, Discount, Amount Due");

			mySimcardsGrid.setInitWidths("*,*,*,*,*");

			mySimcardsGrid.setColAlign("left,left,center,right,right");

			mySimcardsGrid.setColTypes("ro,ro,ro,ro,ro");

			mySimcardsGrid.setColSorting("str,str,str,str,str");

			mySimcardsGrid.init();

			try {
				if(ddata.rows) {
					mySimcardsGrid.parse(ddata,function(){
...
///////////////////////////////////
		...
		myTab.postData('/'+settings.router_id+'/json/', {
			odata: {},
			pdata: "routerid="+settings.router_id+"&action=grid&formid=<?php echo $templatemainid.$submod; ?>grid&module=<?php echo $moduleid; ?>&table=product&rowid=<?php echo !empty($vars['post']['rowid'])?$vars['post']['rowid']:'0'; ?>&formval=%formval%",
		}, function(ddata,odata){

			if(typeof(myWinObj.myProductsGrid)!='null'&&typeof(myWinObj.myProductsGrid)!='undefined'&&myWinObj.myProductsGrid!=null) {
				try {
					myWinObj.myProductsGrid.destructor();
					myWinObj.myProductsGrid = null;
				} catch(e) {
					console.log(e);
				}
			}

			var myProductsGrid = myWinObj.myProductsGrid = new dhtmlXGridObject(myForm.getContainer('stock_products'));

			myProductsGrid.setImagePath("/codebase/imgs/")

			myProductsGrid.setHeader("Product ID, Barcode, Quantity, Discount, Amount Due");

			myProductsGrid.setInitWidths("*,*,*,*,*");

			myProductsGrid.setColAlign("left,left,center,right,right");

			myProductsGrid.setColTypes("ro,ro,ro,ro,ro");

			myProductsGrid.setColSorting("str,str,str,str,str");

			myProductsGrid.init();

			try {
				if(ddata.rows) {
					myProductsGrid.parse(ddata,function(){
